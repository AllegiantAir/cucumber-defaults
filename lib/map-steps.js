...
MapSteps = function () {
  ...
  this.fillIn = function(field, data, callback, self) {
    ...
    helper.waitForElementEnabled(XPath, 5000, 100, function(err) {
      if(err){
        callback.fail(err);
      } else {
  ...
  this.isPresent = function(text, callback, self, present) {
    var XPath;
    XPath = self.namedSelectors.getXPath("content", text);
    self.browser.waitForElementByXPath(XPath, self.asserters.isNotDisplayed, function(err, el) {
      if(helper.exists(el)) {
        if(present == "true"){
