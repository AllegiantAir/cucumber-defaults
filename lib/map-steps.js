    ...
    this.fillIn = function(field, data, self) {
        ...
        return Q.Promise(function(resolve, reject, notify) {
            browser.waitForElementByXPath(XPath, function (err, el) {
              if (err || (el == null) ) {
                console.log('fillIn xpath reject:', err.message);
                reject(err);
              } else {
                helper.waitForElementEnabled(el, 5000, function(err) {
                  if (err){
                    console.log('fillIn enabled reject:', err.message);
                    reject(err);
                  } else {
                    el.clear(function(err) {
                      if (err) {
                        console.log('fillIn clear reject:', err.message);
                        reject(err);
                      } else {
                        browser.type(el, data, function(err) {
                          if (err) {
                            console.log('fillIn type reject:', err.message);
                            reject(err);
                          } else {
                            resolve();
                          }
      ...
      function(resolve, reject) {
        self.browser.url(function(err, url) {
          if (err) {
            ...
            resolve( this.iGoToUrl(url, self) );
          }
        });
      }
    );
