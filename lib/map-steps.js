...
MapSteps = function () {
    ...
    this.fillIn = function(field, data, callback, self) {
        var error;
        var XPathElement = self.namedSelectors.getXPath('field', field);
        var browser = self.browser;

        helper.waitForElementEnabled(XPathElement, 50, 2000, function (err, el) {
            if( helper.exists(err) || !helper.exists(el) ) {
    ...
    this.fieldExists = function(field, callback, negate, self) {
        ...
        helper.waitForElementEnabled(XPathElement, 50, 2000, function(err, ele) {
    ...
    this.fieldContains = function(data, field, callback, negate, self) {
        ...
        browser.waitForElementByXPath(XPathField, function(err, el) {
            if(helper.exists(el) && helper.isElementEnabled(el)) {
    ...
    this.elementExists = function(element, callback, negate, self) {
        ...
        browser.waitForElementByXPath(XPathElement, function(err, ele) {
            if( boolLambda(helper.exists(ele) && helper.isElementEnabled(ele)) ) {
                callback();
            } else {
                callback.fail(error);
            }
    ...
    this.isPresent = function(text, callback, self, present) {
        ...
        self.browser.waitForElementByXPath(xpathIsPresent, function(err, el) {
            if(helper.exists(el)) {
                self.browser.waitForElementByXPath(xpathIsPresent, self.asserters.isNotDisplayed, function(err, el) {
                    if(helper.exists(el) && helper.isElementEnabled(el)) {
                        if(present == "true"){
    ...
    this.theButtonShouldBe = function(button, enDis, callback, self) {
        ...
        browser.waitForElementByXPath(XPath, function (err, el) {
            if (err) {
                callback.fail(err);
            } else if(helper.exists(el) && helper.isElementEnabled(el)) {
    ...
    this.attachFile = function(file, element, callback, self) {
        ...
        self.browser.waitForElementByXPath(XPathFileUpload, function(err, el) {
            if(helper.exists(el) && helper.isElementEnabled(element)) {
    ...
    this.searchAndClick = function(searchText, clickText, callback, self) {
        var xpathSearch = self.namedSelectors.getXPath('node_with_content', searchText);
        var browser = self.browser;
        browser.waitForElementByXPath(xpathSearch, function(err, el) {
            if(err) {
                callback.fail(err);
            } else {
        ...
        function clickableEle(el, text, callback){
            var xpathClick = self.namedSelectors.getXPath('node_with_content', text);
            el.elementByXPath(xpathClick, function(err, ele) {
                if(err) {
