...
MapSteps = function () {
    ...
    this.searchAndClick = function(searchText, clickText, callback, self) {
        var xpath = self.namedSelectors.getXPath('content', searchText);
        var browser = self.browser;
        var xpathSearchText = self.namedSelectors.getXPath('searchText', searchText);

        browser.waitForElementByXPath(xpathSearchText, function(err, el) {
            if(err) {
                callback.fail(err);
            } else {
        ...
        function clickableEle(el, text, callback){
            var xpath = self.namedSelectors.getXPath('content', text);
            var xpathClickText = self.namedSelectors('clickText', text);

            el.elementByXPath(xpathClickText, function(err, ele) {
                if(err) {
