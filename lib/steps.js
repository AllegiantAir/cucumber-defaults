...
exports.Steps = function() {
    ...
    this.Then(
        ...
        function (text, callback) {
            this.browser.waitForElementByXPath(
                    "//*[contains(.,'" + text + "')]",
                function (err) {
                    if (err) {
                        callback.fail(err);
    ...
    this.Then(
        ...
        function (button, enDis, callback) {
            var enabled = ('en' == enDis);
            this.browser.waitForElementByXPath(
                this.namedSelectors.getXpath('button', button),
                function (err, el) {
                    if (err) {
                        callback.fail(new Error("no button (" + button + ") found."));
                    } else {
                        el.isEnabled(function(err, isEnabled) {
                            if (err) {
                                callback.fail(err);
                            } else {
                                if ((enabled && isEnabled) || (!enabled && !isEnabled)) {
                                    ...
                                    callback.fail(new Error("button (" + button + ") should be " + enDis + "abled"));
                                }
                            }
                        });
                    }
