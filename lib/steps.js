...
exports.Steps = function() {
    ...
    this.Then(/^I should see an? "([^"]*)" element$/, function (element, callback) {
        var xpathElement = this.namedSelectors.getXpath('tagname', element);
        var browser = this.browser;
            ...
            browser.waitForElementByXPath(xpathField, function (err, el) {
                if(exists(el)) {
                    el.text(function (err, text) {
                        if(text != data)
                            callback.fail(new Error('Text does not match.'));
                        callback();
                    });
                } else {
                    callback.fail(errorElementDoesNotExists);
        ...
        function (field, text, callback) {
            ...
            browser.waitForElementByXPath(xpathField, function (err, el) {
                if(exists(el)) {
                    el.text(function (err, text) {
                        if(text == data)
                            callback.fail(new Error('Field should not contain: ' + text));
                        callback();
                    });
                } else {
                    callback.fail(errorElementDoesNotExists);
